...
files_size_bytes=0
db_size_bytes=0
if [[ -d "${LOCAL_BACKUP_ROOT:-${OSB_BACKUPS:-$OSB_HOME/data/backups/tbsoftwash-live}}" ]]; then
  latest_backup="$(ls -1dt "${LOCAL_BACKUP_ROOT:-${OSB_BACKUPS:-$OSB_HOME/data/backups/tbsoftwash-live}}"/* 2>/dev/null | head -n1 || true)"
fi
